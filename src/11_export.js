...
J.exportMP4 = async ({ plan, project, audio, quality = 'high', onProgress, signal }) => {
  ...
  if (!vc) throw new Error('This browser does not support video encoding (WebCodecs). Please open the app in the latest Chrome or Edge.');
  let ac = null;
  ...
  for (let i = 0; i < total; i++) {
    if (signal && signal.aborted) { try { venc.close(); } catch (e) {} throw new Error('Canceled'); }
    if (err) throw err;
    ...
    if (i % 3 === 0) { onProgress && onProgress(i / total, `Frame ${i + 1}/${total}`); await new Promise(r => setTimeout(r, 0)); }
  }
  } finally { J.glyphs.maxRes = prevRes; }
  await venc.flush(); venc.close();
  if (ac) {
    onProgress && onProgress(0.99, 'Encoding audio');
...
J.planForAE = (plan, project) => {
  ...
  if (subs) clean.aeNote = `Replaced ${subs} newer browser-only effects with the closest ones available in the AE panel (text treatments, backgrounds, camera moves and cut transitions are not supported in the AE version)`;
