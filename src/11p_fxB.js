...
/* ================= lens / optics ================= */
fx('radialChroma', { name: 'Radial Aberration', tags: ['glitch', 'emotional', 'pop'], w: 0.9, dur: 4, amp: 1, mid: true, scratch: true, ae: 'chroma',
  draw(ctx, ev, k, I) {
...
fx('bloomFlash', { name: 'Bloom', tags: ['pop', 'emotional', 'calm'], w: 1, dur: 6, amp: 1, mid: true, scratch: true, ae: 'flash',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('bulge', { name: 'Fisheye', tags: ['pop', 'graphic', 'glitch'], w: 0.8, dur: 5, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
...
fx('pixelSort', { name: 'Pixel Sort', tags: ['glitch'], w: 0.8, dur: 3, amp: 1, glitchy: true, mid: true, scratch: true, ae: 'slice',
  draw(ctx, ev, k, I) {
    const { cw, ch, S } = I; if (!S) return;
...
fx('interlace', { name: 'Interlace', tags: ['glitch', 'emotional'], w: 0.7, dur: 3, amp: 1, glitchy: true, mid: true, scratch: true, ae: 'slice',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('macroBlock', { name: 'Block Noise', tags: ['glitch'], w: 0.8, dur: 3, amp: 1, glitchy: true, mid: true, scratch: true, ae: 'block',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('halftone', { name: 'Halftone', tags: ['pop', 'graphic', 'editorial'], w: 0.9, dur: 5, amp: 1, mid: true, ae: 'mosaic',
  draw(ctx, ev, k, I) {
    const { cw, ch, sc } = I;
...
fx('duotone', { name: 'Duotone', tags: ['pop', 'emotional', 'graphic'], w: 0.8, dur: 4, amp: 1, mid: true, ae: 'chroma',
  draw(ctx, ev, k, I) {
...
fx('ditherBit', { name: '1-Bit Dither', tags: ['glitch', 'graphic', 'pop'], w: 0.7, dur: 3, amp: 1, mid: true, scratch: true, ae: 'mosaic',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
/* ================= frame motion ================= */
fx('rotateSnap', { name: 'Tilt Snap', tags: ['pop', 'graphic', 'glitch'], w: 0.9, dur: 5, amp: 1, mid: true, scratch: true, ae: 'shake',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('echoFrames', { name: 'Afterimage Echo', tags: ['emotional', 'pop', 'glitch'], w: 0.9, dur: 6, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('kaleido', { name: 'Kaleidoscope', tags: ['pop', 'graphic', 'emotional'], w: 0.6, dur: 4, amp: 1, mid: true, scratch: true, ae: 'block',
  draw(ctx, ev, k, I) {
    const { cw, ch, S } = I; if (!S) return;
...
/* ================= inversion / light ================= */
fx('bandInvert', { name: 'Band Invert', tags: ['glitch', 'graphic'], w: 0.7, dur: 3, amp: 1, glitchy: true, mid: true, ae: 'invert',
  draw(ctx, ev, k, I) {
...
fx('lightRays', { name: 'Light Rays', tags: ['emotional', 'pop', 'calm'], w: 0.9, dur: 8, amp: 1, mid: true, ae: 'flash',
  draw(ctx, ev, k, I) {
...
fx('anamorphic', { name: 'Anamorphic Flare', tags: ['emotional', 'pop', 'calm'], w: 0.8, dur: 7, amp: 1, mid: true, ae: 'flash',
  draw(ctx, ev, k, I) {
    const { cw, ch, sc } = I, s = evS(ev), dk = isDark(sc.bg);
...
fx('heartbeat', { name: 'Heartbeat', tags: ['emotional', 'calm'], w: 0.8, dur: 10, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('tvStatic', { name: 'TV Static', tags: ['glitch', 'emotional'], w: 0.6, dur: 5, pre: 2, amp: 1, glitchy: true, ae: 'block',
  draw(ctx, ev, k, I) {
...
fx('dustScratches', { name: 'Film Scratches', tags: ['emotional', 'calm', 'editorial'], w: 0.8, dur: 8, amp: 1, mid: true,
  draw(ctx, ev, k, I) {
...
fx('filmAdvance', { name: 'Film Advance', tags: ['emotional', 'editorial', 'calm'], w: 0.6, dur: 9, pre: 4, amp: 1, scratch: true, ae: 'slice',
  draw(ctx, ev, k, I) {
    const { cw, ch, S } = I; if (!S) return;
...
fx('perspectiveTilt', { name: 'Perspective Sway', tags: ['pop', 'graphic', 'emotional'], w: 0.8, dur: 8, amp: 1, mid: true, scratch: true, ae: 'shake',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('ripple', { name: 'Ripple', tags: ['emotional', 'calm', 'pop'], w: 0.8, dur: 10, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
// focus lines (manga shuchusen): thin wedges converging on the centre, redrawn every frame like hand-drawn animation
fx('focusLines', { name: 'Focus Lines', tags: ['pop', 'graphic', 'emotional'], w: 0.9, dur: 6, amp: 1, mid: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
...
// speed lines: tapered streaks racing across the frame (clear of the centre band)
fx('speedLines', { name: 'Speed Lines', tags: ['pop', 'graphic'], w: 0.8, dur: 6, amp: 1, mid: true,
  draw(ctx, ev, k, I) {
...
// sparkle ("kira!"): 8-point glints popping in sequence around the lyric band
fx('starGlint', { name: 'Sparkle', tags: ['pop', 'emotional'], w: 0.8, dur: 9, amp: 1, mid: true, ae: 'flash',
  draw(ctx, ev, k, I) {
...
fx('colorBars', { name: 'Color Bars', tags: ['pop', 'graphic', 'glitch'], w: 0.8, dur: 6, amp: 1, mid: true, ae: 'slice',
  draw(ctx, ev, k, I) {
...
// three hard zoom steps (da-da-dan!), each with its own slight focus shift, then snap back
fx('zoomStutter', { name: 'Zoom Stutter', tags: ['pop', 'graphic', 'glitch'], w: 0.8, dur: 6, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
    const { cw, ch, S } = I; if (!S) return;
...
fx('negativeRing', { name: 'Invert Ring', tags: ['graphic', 'pop', 'glitch'], w: 0.8, dur: 6, amp: 1, mid: true, ae: 'invert',
  draw(ctx, ev, k, I) {
...
fx('edgeDetect', { name: 'Edge Detect', tags: ['graphic', 'glitch', 'editorial'], w: 0.7, dur: 4, amp: 1, mid: true, scratch: true, ae: 'invert',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('shatter', { name: 'Shattered Glass', tags: ['glitch', 'pop', 'emotional'], w: 0.5, dur: 8, pre: 1, amp: 1, scratch: true, ae: 'block',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('defocus', { name: 'Defocus', tags: ['emotional', 'calm', 'editorial'], w: 0.9, dur: 8, pre: 4, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
    const { cw, ch, S } = I; if (!S) return;
...
fx('snapshot', { name: 'Shutter', tags: ['pop', 'emotional', 'editorial'], w: 0.6, dur: 10, amp: 1, mid: true, scratch: true, ae: 'flash',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('squash', { name: 'Squash & Stretch', tags: ['pop', 'graphic'], w: 0.8, dur: 6, amp: 1, mid: true, scratch: true, ae: 'zoom',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('scanBar', { name: 'Scan Bar', tags: ['graphic', 'editorial', 'glitch'], w: 0.7, dur: 8, amp: 1, mid: true, scratch: true, ae: 'flash',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
...
fx('loopScroll', { name: 'Side Loop', tags: ['pop', 'graphic', 'glitch'], w: 0.7, dur: 6, pre: 3, amp: 1, scratch: true, ae: 'slice',
  draw(ctx, ev, k, I) {
    const { cw, ch, S, sc } = I; if (!S) return;
